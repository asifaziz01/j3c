...
const appPath = 'http://localhost/repos/j3c/';
const rootPath = 'http://localhost/repos/j3c/';
const appTitle = 'Just3Click';
const notifyIcon = `${appPath}template/default/images/touch/icon192.png`;
const notifyBadge = `${appPath}template/default/images/touch/icon128.png`;

const staticAssets = [
	appPath,
	appPath + 'template/default/css/',
	appPath + 'template/default/js/',
	appPath + 'template/default/images/',
];

self.addEventListener ('install', async event => {
	const cache = await caches.open ('J3C');
...
async function networkFirst (request) {
	const cache = await caches.open ('J3C-V1-dynamic');
	if(request.url.search("chrome-extension:") < 0){
		try {
